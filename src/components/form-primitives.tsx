...
export function FormGrid({ cols = 3, children, className }: FormGridProps) {
  const map = { 1: "md:grid-cols-1", 2: "md:grid-cols-2", 3: "md:grid-cols-3", 4: "md:grid-cols-4" } as const;
  return <div className={cn("grid grid-cols-1 gap-x-4 gap-y-3", map[cols], className)}>{children}</div>;
}
...
export function Field({
  label, hint, children, className, required,
}: FieldProps) {
  return (
